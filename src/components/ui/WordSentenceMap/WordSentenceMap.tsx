...
import type { WordSentenceMapProps, WordTokenProps } from './WordSentenceMap.types';
import { useWordSentenceMap } from './useWordSentenceMap';

const UNDERLINE_CLASSES: Record<'none' | 'amber' | 'red', string> = {
  none: '',
  amber: 'underline decoration-amber-400 decoration-2 underline-offset-2 cursor-pointer hover:decoration-amber-600',
  red: 'underline decoration-red-500 decoration-2 underline-offset-2 cursor-pointer hover:decoration-red-700',
} as const;
...
// A single word token: interactive button with tooltip when it has pronunciation feedback
function WordToken({ annotatedWord, isOpen, onOpen, onClose }: WordTokenProps) {
  const { word, underlineLevel, tooltip, index } = annotatedWord;
        ...
        >
          ...
          <dl className="space-y-1.5 text-xs">
            ...
            <div className="pt-1 border-t border-gray-100 dark:border-gray-700">
              <dd className="text-gray-700 dark:text-gray-300 leading-snug">{tooltip.tip}</dd>
            </div>
          </dl>
        </div>
      )}
...
export function WordSentenceMap({ words, animationDelay }: WordSentenceMapProps): React.JSX.Element {
  ...
  return (
    <section
      ref={containerRef}
      className="opacity-0"
      style={{ animation: 'fadeInUp 0.5s ease-out forwards', animationDelay: `${animationDelay}ms` }}
    ...
    >
      ...
      <div className="text-base leading-loose flex flex-wrap gap-x-1 gap-y-1">
        {annotatedWords.map((annotatedWord) => (
          <WordToken
            key={`${annotatedWord.word.word}-${annotatedWord.index}`}
            annotatedWord={annotatedWord}
            isOpen={openIndex === annotatedWord.index}
            onOpen={openTooltip}
