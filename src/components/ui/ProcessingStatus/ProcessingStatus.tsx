...
function FailedView({ errorMessage, onRetry }: FailedViewProps) {
  return (
    <div role="alert" aria-live="assertive" className="rounded-lg border border-red-200 bg-red-50 p-6">
...
function StepsList({ currentIndex }: StepsListProps) {
  return (
...
interface ProgressiveStepsProps {
  partialData: NonNullable<ProcessingStatusProps['partialData']>;
}

function ProgressiveSteps({ partialData }: ProgressiveStepsProps) {
  const steps = [
      ...
      <ul className="space-y-2">
        {steps.map((step) => (
          <li key={step.key} className="flex items-center gap-2 text-sm">
            <span
              className={step.ready ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'}
              aria-hidden
            >
              {step.ready ? '✓' : '…'}
            </span>
...
export function ProcessingStatus({ status, errorMessage, onRetry, partialData }: ProcessingStatusProps) {
  ...
  if (displayStatus === 'FAILED') {
    return <FailedView errorMessage={errorMessage} onRetry={onRetry} />;
  }
  ...
  return (
    <div
      className="rounded-lg border border-blue-200 bg-blue-50 p-6 dark:border-slate-700 dark:bg-slate-800/80"
      aria-live="polite"
      role="status"
    >
      <h3 className="text-lg font-semibold text-blue-900 dark:text-sky-300">
        Processing Your Workout
      </h3>
      <StepsList currentIndex={currentIndex} />
      {partialData !== undefined && <ProgressiveSteps partialData={partialData} />}
    </div>
  );
}
