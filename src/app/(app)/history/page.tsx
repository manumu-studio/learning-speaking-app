// Session history page — activity feed with Suspense boundary for search param reads
'use client';

import { Suspense, useCallback, useState } from 'react';
import { Container } from '@/components/ui/Container';
import { MicLoadingIndicator } from '@/components/ui/MicLoadingIndicator';
import { DeleteSessionModal } from '@/components/ui/DeleteSessionModal';
import { useSessionHistory } from '@/features/session/useSessionHistory';
import { HistoryDateFilter } from './HistoryDateFilter';
import { HistoryEmpty, HistorySessionList } from './HistorySessionList';

function HistorySkeleton() {
  return (
    <Container>
      <div className="mb-6 h-8 w-32 animate-pulse rounded bg-gray-200 dark:bg-gray-800" />
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          key={i}
          className="mb-3 h-16 animate-pulse rounded-xl bg-gray-100 dark:bg-gray-900"
        />
      ))}
    </Container>
  );
}

function HistoryContent() {
  const {
    dayGroups,
    isLoading,
    isFetchingMore,
    error,
    hasMore,
    total,
    dateFilter,
    setDateFilter,
    sentinelRef,
    removeSession,
  } = useSessionHistory();

  const [deleteTargetId, setDeleteTargetId] = useState<string | null>(null);
  const [showDeleteToast, setShowDeleteToast] = useState(false);

  const handleDeleteRequest = useCallback((sessionId: string) => {
    setDeleteTargetId(sessionId);
  }, []);

  const handleDeleteConfirm = useCallback(() => {
    if (deleteTargetId === null) return;
    removeSession(deleteTargetId);
    setDeleteTargetId(null);
    setShowDeleteToast(true);
    setTimeout(() => setShowDeleteToast(false), 3000);
  }, [deleteTargetId, removeSession]);

  const handleDeleteClose = useCallback(() => {
    setDeleteTargetId(null);
  }, []);

  const showEmpty = !isLoading && (error !== null || dayGroups.length === 0);
  const showList = !isLoading && error === null && dayGroups.length > 0;

  return (
    <Container>
      <div className="flex items-baseline justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
          Activity
        </h1>
        {total > 0 && (
          <span className="text-sm text-gray-400 dark:text-gray-500">
            {total} session{total !== 1 ? 's' : ''}
          </span>
        )}
      </div>

      <HistoryDateFilter dateFilter={dateFilter} onFilterChange={setDateFilter} />

      {isLoading && (
        <div className="flex items-center justify-center pt-32">
          <MicLoadingIndicator />
        </div>
      )}

      {showEmpty && <HistoryEmpty error={error} />}

      {showList && (
        <HistorySessionList
          dayGroups={dayGroups}
          hasMore={hasMore}
          isFetchingMore={isFetchingMore}
          sentinelRef={sentinelRef}
          onDeleteSession={handleDeleteRequest}
        />
      )}

      {deleteTargetId !== null && (
        <DeleteSessionModal
          isOpen
          sessionId={deleteTargetId}
          onClose={handleDeleteClose}
          onConfirm={handleDeleteConfirm}
        />
      )}

      {showDeleteToast && (
        <div className="fixed bottom-20 left-1/2 z-50 -translate-x-1/2 animate-fade-in-out rounded-full bg-gray-900 px-4 py-2 text-sm font-medium text-white shadow-lg dark:bg-gray-100 dark:text-gray-900">
          Session deleted
        </div>
      )}
    </Container>
  );
}

export default function HistoryPage() {
  return (
    <Suspense fallback={<HistorySkeleton />}>
      <HistoryContent />
    </Suspense>
  );
}
