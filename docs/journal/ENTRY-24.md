...
1. **CI completeness** — Added the test step (`npm run test`), wired up coverage reporting with a 20/40% floor, added `npm audit` to catch known vulnerabilities in CI, and added a bundle budget check (120 kB shared JS) so dependency bloat fails the build before it reaches production.
...
| `vitest.config.ts` | Added coverage config (v8 provider, 20/40% thresholds) |
...
**Why 20/40% coverage thresholds?**
Statements and lines are at 20% — the honest floor given the current test surface covers only 6 files while coverage measures all of `src/lib/` and `src/features/`. Branches (84.96%) and functions (92.06%) pass comfortably at 40%. These thresholds ratchet up as PACKET-26/27 add unit and integration tests.
...
- Coverage thresholds (20% statements/lines, 40% branches/functions) are a floor, not a goal. Future test work should push these toward 70%+.
